...
const Hero = () => {
  ...
  return (
    <section className="relative overflow-hidden pt-32 md:pt-36 pb-20 bg-gray-950 text-white" aria-labelledby="hero-title">
      ...
      {/* --- SOLUCIÓN DEFINITIVA: CSS PURO Y SIMPLE --- */}
      <style>{`
        .hero-bg-animation {
          display: none; /* Por defecto, está OCULTO en móvil */
        }
        @media (min-width: 768px) { /* A partir de 768px (tablets/desktop)... */
          .hero-bg-animation {
            display: block; /* ...se MUESTRA */
            animation: fadeInOut 2.5s ease-out forwards;
          }
        }
        @keyframes fadeInOut { 0%, 100% { opacity: 0; } 50% { opacity: 0.2; } }
      `}</style>
      <div className="hero-bg-animation absolute inset-0 opacity-0">
